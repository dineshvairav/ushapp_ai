...
const TARGET_STORAGE_BUCKET = "gs://ushapp-af453.firebasestorage.app"; // For general admin uploads
...
export interface GuestSharedFile extends Omit<AdminUploadedFile, 'uploadedByEmail'> {
  targetPhoneNumber?: string; // If we store it, for admin reference
}
...
export default function FileUploadPage() {
  ...
  // State for general admin uploads
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [uploadProgress, setUploadProgress] = useState(0);
  const [isUploading, setIsUploading] = useState(false);

  // State for guest-specific uploads
  ...
  const [uploadedFiles, setUploadedFiles] = useState<AdminUploadedFile[]>([]); // General admin files
  ...
  const handleFileUpload = async (e: FormEvent<HTMLFormElement>) => {
    ...
    uploadTask.on('state_changed',
      ...
      (error) => {
        ...
        toast({ variant: "destructive", title: "Upload Failed", description: `Upload failed: ${error.code} - ${error.message}. Check Storage rules.` });
  ...
  const handleGuestFileUpload = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const storage = getStorage(firebaseApp); // Use default bucket, path will specify folder
    ...
    uploadTask.on('state_changed',
      ...
      (error) => {
        ...
        toast({ variant: "destructive", title: "Upload Failed", description: `Upload failed: ${error.code} - ${error.message}. Check Storage rules for guestSharedFiles.` });
      ...
      async () => {
        try {
          ...
          const fileData: Omit<GuestSharedFile, 'id'> = {
            ...
            uploadedAt: new Date().toISOString(),
            targetPhoneNumber: sanitizedPhoneNumber, // Optional: store for admin reference
          };
          await set(newFileRef, fileData);
          ...
          if (guestFileInput) guestFileInput.value = '';
            ...
            console.error("Error saving guest file metadata to RTDB:", dbError);
  ...
  const handleDeleteFile = async (file: AdminUploadedFile) => {
    ...
    try {
      const storage = getStorage(firebaseApp, TARGET_STORAGE_BUCKET); // Use specific bucket for general admin files
      ...
      const storagePath = decodeURIComponent(pathSegments[1].split('?')[0]);
      if (!storagePath) throw new Error("Storage path for deletion is empty.");
    ...
    <MainAppLayout>
      ...
      <Card className="mb-8 border-destructive/30 bg-destructive/5">
        ...
        <CardContent className="text-sm text-destructive/80 space-y-1 pl-12">
            <p>Files are stored in Firebase Storage and metadata in RTDB. Ensure Security Rules are configured for `adminUploads/` (general) and `guestSharedFiles/` (guest-specific) paths.</p>
        </CardContent>
      </Card>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Card for Uploading Guest-Specific File */}
        <Card className="lg:col-span-1 border-border shadow-lg">
          ...
          <form onSubmit={handleGuestFileUpload}>
            ...
            <CardFooter>
              <Button type="submit" className="w-full bg-primary hover:bg-primary/90 text-primary-foreground" disabled={isUploadingGuestFile || !selectedGuestFile || !guestPhoneNumber}>
        ...
        {/* Card for General Admin File Upload */}
        <Card className="lg:col-span-1 border-border shadow-lg">
          <CardHeader>
        ...
        <div className="lg:col-span-3"> {/* Changed to lg:col-span-3 to take full width on large screens if only this card is left, or make it span 2 if you prefer that layout */}
          {isLoadingFiles && (
